...
using TMPro;
using UnityEngine;
public class WeaponKeepsStats : MonoBehaviour
{
    ...
    private IEnumerator Shoot()
    {
        ...
        if (currentAmmo > 0)
        {
            PlayerPerformanceManager.Instance.UpdateShotsFired();
    ...
    private void ProcessRayCast()
    {
        if (Physics.Raycast(fpsCamera.transform.position, fpsCamera.transform.forward, out hit, range))
        {
            ...
            {
                PlayerPerformanceManager.Instance.UpdateShotsHit();
                if (hit.transform.tag == "Head")
                {
                    SoundManager.Instance.PlayAtLocation(headShotSound, hit.point);
                    PlayerPerformanceManager.Instance.UpdateHeadShotsHit();
                    PlayerPerformanceManager.Instance.UpdateHeadShotDistance(hit.distance);
                    targetHealthcomponent.TakeDamage(damage * headshotMultiplier);
                }
                else if (hit.transform.tag == "Enemy")
                {
                    PlayerPerformanceManager.Instance.UpdateHeadShotStreak(false);
    ...
    {
        // Use the object pool to spawn the hit effect
